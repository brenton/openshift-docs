...
This is mostly because some DNS client libraries will not properly fail over to
the other available Masters for cluster DNS.  In addition it stopping the
Master and Controllers services you can remove the EndPoint from the kubernetes
Service's `subsets.addresses`.
...
which the new deployment can be installed.  After the new deployment is
verified traffic can be switched over with the option to rollback should a
problem be detected.
...
deployments.  In an OpenShift environment the most suitable candidate for
blue-green deployments are the Nodes.  Uptime is most important for these
workloads and the additional complexity of blue-green deployments is can be
...
can be accomplished by shareing the consumer ID from the previous host.
...
[IMPORTANT]
====
When the deployment has been declared successful it's important that the old
host be unregistered with `subscription-manager clean` to avoid creating a
situation where the environment is out of compliance.
====
...
link:#upgrading-masters[After the Master and etcd servers have been upgraded]
you will need to ensure that your current production Nodes as either blue or
green.  In this example the currently installation environment will be blue and
the new environment will be green.

----
oc label --all nodes color=blue
----

There's quite a bit of flexibility here.  Perhaps there was only some subset of
...
Next link:../install/advanced_install.html#adding-nodes-advanced[the green
...
Nodes.  Pointing the deployment configs to the new image will cause a rolling
...
delays if there isn't sufficient capacity built in to the environment. Often
...
Quite often a major release of OpenShift is the motivation for a blue-green
deployment.  In that case there will be new ImageStreams available for users of
...
builds.  The good news is that they will all be performed on the green Nodes
and therefore not impact any traffic on the blue deployment.
...
build images, pod infrastructure image and deployers.  Everything else can be
moved over using Node evacuation and will proceed more quickly as a result.

=== Node evacuation

For larger deployments it's quite possible to have other labels that help
determine how evacuation can be coordinated.  The most conservative approach
for avoiding downtime would be to evacuate one Node at a time.  If services are
composed of Pods using Zone anti-affinity then an entire zone could be
evacuated at once.  It's important to ensure that the storage volumes used are
...
stopped.  Manual evacuation and deletion of all blue Nodes at once could be
achieved as follows:
...
In the OpenShift environment the most suitable candidate for blue-green
deployments is the Node tier.  All user processes run on these systems and even
critical pieces of OpenShift infrastructure is self-hosted there.  Uptime is
most important for these workloads and the additional complexity of blue-green
deployments is can be justified.  The exact implementation of this approach
will vary based on your requirements.
